...
scripts are executable. Skills keep outputs in established repository formats;
custom schema or generated
...
```bash
bunx --yes skills add xsyetopz/.agents \
  --skill repository-docs --agent codex --copy -y

bunx --bun skills add xsyetopz/.agents \
  --skill repository-docs --agent codex --copy -y
```
...
```bash
bunx --yes skills list --agent codex --json
...
```bash
bunx --yes skills remove --skill repository-docs --agent codex -y
bunx --bun skills remove --skill repository-docs --agent codex -y
```
...
Every `SKILL.md` uses the same One Way Only entrypoint order: **Start with
evidence**, **Workflow**, **Validation**, then **Boundaries**. Skills must remain
self-contained and cannot name a companion skill as a required dependency.
...
uvx ruff check --isolated skills scripts
