...
1. Read [the platform matrix](references/platform-matrix.md) and recommend the narrowest viable platform set with explicit exclusions and uncertainty.
2. For multi-editor work, read [cross-editor architecture](references/cross-editor-architecture.md). Separate reusable protocol/domain cores from thin host adapters without forcing lowest-common-denominator UX.
3. For an existing extension port or migration, read [migration playbook](references/migration-playbook.md). Inventory user-visible and persistent contracts before mapping APIs.
4. Read [cross-editor repositories](references/templates-and-ecosystem.md) when selecting a shared LSP, DAP, grammar, protocol, or monorepo pattern. Treat repositories as evidence, not copy sources.

If a matching platform-specific workflow is already available, it may be used for implementation. Otherwise continue directly from repository evidence, this skill's references, and current official platform documentation. Never stop, search for, or ask the user to install a missing companion skill.
...
- The concrete platform implementation workflow to use next, whether supplied by an installed skill or performed directly.
