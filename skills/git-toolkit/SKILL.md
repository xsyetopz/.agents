---
name: git-toolkit
description: Local Git inspection, history, worktrees, staging, recovery, patches, and commits with explicit scope and recovery evidence.
---

# Git Toolkit

Inspect and change a local Git repository with explicit scope, recovery evidence, and clean status. Hosted settings and provider APIs are boundaries, not local proof.
...
- Handle local rebases, cherry-picks, bisects, stashes, worktrees, patches, hooks, signing, submodules, or Git Large File Storage (Git LFS).
...
- Local command output proves only the inspected checkout. Mark signatures, hooks, LFS downloads, hosted settings, and remote effects `UNVERIFIED` unless directly observed.
...
3. Load only the narrowest operation guide from `references/index.md` and check the source map for current command semantics.
...
6. Re-read status, history, and recovery state; report exact results, source freshness, and remaining risk.

## Resources

- Start with the package-local [reference router](references/index.md).
- Use the package-local [source map](references/sources.md) for Git standards and hosted-boundary references.
...
- Report commands, exit codes, changed paths, evidence, and remaining limits. Mark remote effects, unavailable hooks, signing, LFS objects, repository tests, or other unrun evidence `UNVERIFIED`.
