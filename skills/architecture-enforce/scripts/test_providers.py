...
class ArchitectureToolsTests(unittest.TestCase):
    ...
    def fake_provider(self, output: str, *, exit_code: int = 0, sleep: float = 0) -> str:
    ...
    def test_valid_stream_normalizes_ranges_and_preserves_argv(self) -> None:
        ...
        self.assertEqual((result.findings[0].start_line, result.findings[0].start_column), (1, 1))
    ...
    def test_empty_success_is_pass(self) -> None:
        ...
        result = run_ast_grep(self.root, rule_id="clean", language="python", pattern="pass", severity="warning", message="match", executable=executable)
        self.assertEqual(result.status, "passed")

    def test_missing_provider_is_blocked(self) -> None:
        result = run_ast_grep(self.root, rule_id="missing", language="python", pattern="pass", severity="error", message="match", executable=str(self.root / "missing"))
    ...
    def test_malformed_and_duplicate_output_never_passes(self) -> None:
        ...
        for executable in (malformed, duplicate):
            result = run_ast_grep(self.root, rule_id="bad", language="python", pattern="pass", severity="warning", message="match", executable=executable)
    ...
    def test_outside_path_is_rejected(self) -> None:
        ...
        result = run_ast_grep(self.root, rule_id="escape", language="python", pattern="pass", severity="warning", message="match", executable=executable)
        self.assertEqual(result.status, "invalid-output")
    ...
    def test_timeout_is_distinct(self) -> None:
        ...
        result = run_ast_grep(self.root, rule_id="slow", language="python", pattern="pass", severity="warning", message="match", timeout_s=0.01, executable=executable)
