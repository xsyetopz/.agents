"""Fail-closed detection of lint and check suppression constructs."""
...
_HASH_COMMENT_SUFFIXES = {
    ".bash", ".cfg", ".conf", ".fish", ".ini", ".jl", ".nim", ".pl", ".pm", ".py",
    ".pyi", ".pyw", ".r", ".rake", ".rb", ".sh", ".t", ".toml", ".yaml", ".yml", ".zsh",
}
...
_COMMENT_DIRECTIVES: tuple[tuple[str, re.Pattern[str]], ...] = (
    ("eslint", re.compile(r"\beslint-disable(?:-(?:next-)?line)?(?=$|[\s:*#/])", re.IGNORECASE)),
    ("stylelint", re.compile(r"\bstylelint-disable(?:-(?:next-)?line)?\b", re.IGNORECASE)),
    ...
    ("golangci-lint", re.compile(r"\bgolangci-lint(?:\s+|\s*:\s*)disable\b", re.IGNORECASE)),
...
_CODE_SUPPRESSIONS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ("pragma-warning", re.compile(r"^\s*#\s*pragma\s+warning\s+disable\b", re.IGNORECASE)),
    ("pragma-diagnostic", re.compile(r"^\s*#\s*pragma\s+\w+\s+diagnostic\s+ignored\b", re.IGNORECASE)),
    ("suppress-annotation", re.compile(r"^\s*@Suppress(?:Warnings)?\s*\(", re.IGNORECASE)),
...
_CI_BYPASS = re.compile(r"^\s*(?:continue-on-error|allow_failure)\s*:\s*(?:true|yes|1)\b", re.IGNORECASE)
...
_IGNORE_FILE_NAMES = {
    ".eslintignore", ".stylelintignore", ".biomeignore", ".prettierignore", ".ruffignore",
    ".mypyignore", ".pylintignore", ".flake8ignore", ".pyrightignore", ".tslintignore",
    ".golangciignore", ".rubocopignore", ".swiftlintignore", ".ktlintignore", ".clippyignore",
}
...
_SCRIPT_SUFFIXES = {".bash", ".fish", ".js", ".mjs", ".cjs", ".ps1", ".py", ".rb", ".sh", ".ts", ".zsh"}
_SCRIPT_MANIFEST_NAMES = {
    "build.gradle", "build.gradle.kts", "build.xml", "cargo.toml", "composer.json", "gemfile",
    "justfile", "makefile", "package.json", "pom.xml", "pyproject.toml", "rakefile",
    "taskfile.yml", "taskfile.yaml",
}
_SCRIPT_DIRECTORY_NAMES = {".circleci", ".github", ".gitlab", "bin", "build", "ci", "scripts", "tools"}
...
_CHECK_EXIT_ZERO = re.compile(r"(?i)\b(?:ruff|eslint|stylelint|biome|mypy|pyright|pylint|flake8|golangci-lint|rubocop|swiftlint|ktlint)\b[^\n]*\s--exit-zero\b")
_DOWNGRADED_RULE = re.compile(r"(?ix)[\"']?[\w/@.*:-]+[\"']?\s*:\s*[\"']?(?:warn|warning)[\"']?(?:\s*[,}])")
_CI_DISABLED = re.compile(r"^\s*if\s*:\s*(?:false|\$\{\{\s*false\s*\}\})\s*$", re.IGNORECASE)
_RULE_KEY_ONLY = re.compile(r"^\s*[\"']?[\w/@.*:-]+[\"']?\s*:\s*(?:#.*)?$")
_RULE_DISABLED_VALUE = re.compile(r"^\s*[\"']?(?:off|0|false)[\"']?\s*[,}]?\s*(?:#.*)?$", re.IGNORECASE)
...
def _code_without_strings_or_comments(line: str, suffix: str) -> str:
    ...
    while index < len(line):
        ...
        if char in {"'", '"', "`"}:
            ...
            index += 1
            continue
        if (suffix in _HASH_COMMENT_SUFFIXES and char == "#") or (suffix in _DASH_COMMENT_SUFFIXES and line.startswith("--", index)) or (suffix in _PERCENT_COMMENT_SUFFIXES and char == "%"):
            break
        output.append(char)
        index += 1
    return "".join(output)
...
def _is_workflow(path: Path) -> bool:
    ...
    return (
        ...
        or name in {
            "appveyor.yml", "azure-pipelines.yml", "azure-pipelines.yaml", "bitbucket-pipelines.yml",
            "buildkite.yml", "ci.yml", "ci.yaml", "jenkinsfile", ".travis.yml",
        }
...
def _is_linter_config(path: Path) -> bool:
    ...
    return (
        ...
        or name.startswith((".eslintrc", ".stylelintrc", "tsconfig"))
        or name in {".flake8", "biome.json", "biome.jsonc", "mypy.ini", "pyproject.toml"}
        or name.startswith(".golangci.")
        or "phpstan" in name
    )
...
def _is_relevant_gitignore_pattern(line: str) -> bool:
    ...
    return bool(_GITIGNORE_RELEVANT_PATH.search(pattern) or _GITIGNORE_SOURCE_EXTENSION.search(pattern))
...
def _config_suppression(path: Path, line: str, section: str) -> str | None:
    ...
    for match in _CONFIG_KEY.finditer(line):
        ...
        if name.startswith("tsconfig") and key == "skiplibcheck" and re.match(r"(?:true|1|yes)\b", value):
            return "TypeScript library-check suppression"
        if "golangci" in name and key == "disable_all" and re.match(r"(?:true|1|yes)\b", value):
            return "golangci-lint disable-all setting"
        if ("mypy" in name or "mypy" in section) and key in {"ignore_errors", "ignore_missing_imports"} and re.match(r"(?:true|1|yes)\b", value):
            return "mypy error suppression setting"
        if (name in {".flake8", "flake8", "setup.cfg"} or "flake8" in section) and key in {"ignore", "extend_ignore"} and value:
            return "Flake8 ignored-rule setting"
        if "ruff" in section or "ruff" in name:
            if key in {"ignore", "extend_ignore", "exclude"} and value:
                return "Ruff rule-selection/ignore setting"
        if "eslint" in section or "eslint" in name:
            if key in {"ignores", "ignorepatterns"} and value:
        ...
        if key in {"ignorepatterns", "ignore_pattern", "disable_all", "skip_lib_check"} and value:
...
def _finding(path: Path, line_number: int, message: str, code: str = "lint-suppression") -> Finding:
...
def suppression_findings(path: Path, root: Path | None = None) -> list[Finding]:
    ...
    try:
        ...
        return [_finding(path, 1, f"suppression scan failed: {exc}", "suppression-scan-failed")]
    ...
    if path.name.lower() == ".gitignore":
        for line_number, line in enumerate(lines, 1):
            if _is_relevant_gitignore_pattern(line):
                findings.append(_finding(path, line_number, f"source/test/check/lint path is ignored: {line.strip()}", "gitignore-source-pattern-added"))
    elif _is_ignore_file(path):
        for line_number, line in enumerate(lines, 1):
            if not _is_comment_or_blank(line):
                findings.append(_finding(path, line_number, f"lint/check ignore pattern is active: {line.strip()}", "ignore-pattern-added"))
    if "baseline" in path.name.lower():
        for line_number, line in enumerate(lines, 1):
            if not _is_comment_or_blank(line):
                findings.append(_finding(path, line_number, f"baseline suppression configuration is active: {line.strip()}", "baseline-suppression"))
                break
    for index, line in enumerate(lines):
        ...
        if package_manifest:
            ...
            if scripts_match:
                ...
                package_script_depth = max(0, _json_brace_delta(line[scripts_match.end():]))
        ...
        if comment is not None and any(token in comment.lower() for token in ("disable", "ignore", "expect-error", "nolint", "noqa", "nowarn")):
            for label, pattern in _COMMENT_DIRECTIVES:
                if pattern.search(comment):
                    findings.append(_finding(path, line_number, f"{label} suppression directive: {comment.strip()}"))
                    break
        if suffix == ".rs" and "allow" in line and _RUST_ALLOW.search(line):
            findings.append(_finding(path, line_number, f"Rust lint allowance: {line.strip()}"))
        if "pragma" in line.lower() or "suppress" in line.lower() or "nowarn" in line.lower():
            for label, pattern in _CODE_SUPPRESSIONS:
                if pattern.search(line):
                    findings.append(_finding(path, line_number, f"{label} suppression: {line.strip()}"))
                    break
        code = _code_without_strings_or_comments(line, suffix)
        if "||" in code and _CHECK_BYPASS.search(code):
            findings.append(_finding(path, line_number, f"lint/check/test command is forced successful with `|| true` or `|| :`: {line.strip()}", "check-bypass"))
        if "exit-zero" in code and _CHECK_EXIT_ZERO.search(code):
            findings.append(_finding(path, line_number, f"lint/check provider is forced successful with `--exit-zero`: {line.strip()}", "check-bypass"))
        if package_manifest and package_script_line and _PACKAGE_BYPASS.search(line):
            findings.append(_finding(path, line_number, f"package lint/check/test script is forced successful: {line.strip()}", "check-bypass"))
        if workflow and _CI_BYPASS.search(line):
            context = "\n".join(lines[max(0, index - 8): index + 9])
            if _CI_CONTEXT.search(context):
                findings.append(_finding(path, line_number, f"CI lint/check/test step allows failure: {line.strip()}", "check-bypass"))
        if workflow and _CI_DISABLED.search(line):
            context = "\n".join(lines[max(0, index - 8): index + 9])
            if _CI_CONTEXT.search(context):
                findings.append(_finding(path, line_number, f"CI lint/check/test step is disabled: {line.strip()}", "check-bypass"))
        ...
        if linter_config and not path.name.lower().startswith("tsconfig") and any(token in lowered_line for token in ("off", "false", ": 0", ":0")) and _DISABLED_RULE.search(line):
            findings.append(_finding(path, line_number, f"linter rule severity is disabled: {line.strip()}", "lint-severity-disabled"))
        if linter_config and not path.name.lower().startswith("tsconfig") and "warn" in lowered_line and _DOWNGRADED_RULE.search(line):
            findings.append(_finding(path, line_number, f"linter rule severity is downgraded: {line.strip()}", "lint-severity-downgraded"))
        if linter_config:
            config_message = _config_suppression(path, line, section)
            if config_message is not None:
                findings.append(_finding(path, line_number, f"{config_message}: {line.strip()}", "lint-config-suppression"))
            if _multiline_disabled_rule(lines, index):
                findings.append(_finding(path, line_number, f"linter rule severity is disabled on a continuation line: {line.strip()}", "lint-severity-disabled"))
    return findings
