...
def _run_git_rename_inventory(repository: Path, *, cached: bool) -> bytes:
    ...
    command = [
        "git", "-C", str(repository), "diff", "--no-ext-diff", "--no-color",
        f"--find-renames={_RENAME_SIMILARITY_THRESHOLD}", "--diff-filter=R",
        "--name-status", "-z", "--no-prefix",
    ]
    if cached:
        command.append("--cached")
    try:
        ...
        raise GitInventoryError(f"Git rename inventory failed: {exc}") from exc
    if result.returncode != 0:
        detail = os.fsdecode(result.stderr).strip() or f"exit status {result.returncode}"
        raise GitInventoryError(f"Git rename inventory failed: {detail}")
    return result.stdout


def _run_git_no_rename_inventory(repository: Path, *, cached: bool) -> bytes:
    """Return added/deleted paths without asking Git to lower rename evidence."""

    command = [
        "git", "-C", str(repository), "diff", "--no-ext-diff", "--no-color",
        "--no-renames", "--diff-filter=AD", "--name-status", "-z", "--no-prefix",
    ]
    if cached:
        command.append("--cached")
    try:
        ...
        raise GitInventoryError(f"Git path inventory failed: {exc}") from exc
    if result.returncode != 0:
        detail = os.fsdecode(result.stderr).strip() or f"exit status {result.returncode}"
        raise GitInventoryError(f"Git path inventory failed: {detail}")
    return result.stdout
...
def _git_added_deleted_paths(payload: bytes, repository: Path) -> tuple[set[Path], set[Path]]:
...
def _git_blob(repository: Path, path: Path, *, cached: bool, old: bool) -> bytes | None:
    ...
    try:
        ...
        return None
    if not cached and not old:
        try:
            return path.read_bytes()
        except OSError:
            return None
    revision = "HEAD" if cached and old else ":"
...
def _relocation_similarity(old: bytes, new: bytes, old_path: Path, new_path: Path, repository: Path) -> float:
    old_lines = _normalise_relocated_lines(old, old_path, repository)
    new_lines = _normalise_relocated_lines(new, new_path, repository)
    return difflib.SequenceMatcher(None, old_lines, new_lines, autojunk=False).ratio() * 100
    ...
    *,
    ...
    for old in deleted:
        ...
        if len(matches) != 1:
            continue
        new = matches[0]
        if not _is_destructive_path(old, repository) or not _is_destructive_path(new, repository):
            continue
        ...
        if old_blob is None or new_blob is None:
            continue
        if _relocation_similarity(old_blob, new_blob, old, new, repository) >= _RENAME_SIMILARITY_THRESHOLD:
...
def _provider_lines(value: bytes) -> list[tuple[int, str]]:
    return [
        (line_number, line)
        for line_number, line in enumerate(value.decode("utf-8", errors="replace").splitlines(), 1)
