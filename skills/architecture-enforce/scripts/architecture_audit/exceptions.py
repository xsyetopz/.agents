...
def _config_path_is_visible(root: Path, path: Path, inventory: Sequence[Path] | None) -> bool:
    """Return whether a repository-local config path is in the audit inventory."""
    ...
    return any(candidate.absolute() == target for candidate in inventory)
...
) -> tuple[list[NamingException], list[ArtifactException], list[Finding]]:
    if path is None:
        path = root / ".architecture-enforcement.json"
    if not _config_path_is_visible(root, path, inventory) or not path.exists():
        return [], [], []
    try:
...
def load_test_source_roots(
    root: Path,
    path: Path | None,
    *,
    inventory: Sequence[Path] | None = None,
) -> tuple[list[TestSourceRoot], list[Finding]]:
...
def load_syntax_rules(
    ...
    *,
    inventory: Sequence[Path] | None = None,
) -> tuple[list[SyntaxRule], list[Finding]]:
