...
This compatibility façade keeps the historical ``architecture_audit`` module
and monkeypatchable helper names stable while diff and inventory mechanics live
in focused package-local modules.
"""
...
from . import suppressions as _suppression
from .discovery import GitInventoryError, git_repository_root
from .records import Finding

# Keep all historical helper attributes patchable from this façade.
_candidate_rename_pairs = _diff._candidate_rename_pairs
_decode_git_path = _diff._decode_git_path
_git_added_deleted_paths = _diff._git_added_deleted_paths
_git_blob = _diff._git_blob
_git_diff_events = _diff._git_diff_events
_git_rename_pairs = _diff._git_rename_pairs
_is_destructive_path = _diff._is_destructive_path
_is_script_path = _diff._is_script_path
_normalise_relocated_lines = _diff._normalise_relocated_lines
_provider_invocation = _diff._provider_invocation
_provider_lines = _diff._provider_lines
_provider_removals_for_rename = _diff._provider_removals_for_rename
_relocation_similarity = _diff._relocation_similarity
_run_git_diff = _diff._run_git_diff
_run_git_no_rename_inventory = _diff._run_git_no_rename_inventory
_run_git_rename_inventory = _diff._run_git_rename_inventory
_untracked_candidates = _inventory._untracked_candidates
_untracked_paths = _inventory._untracked_paths
_DiffEvent = _suppression._DiffEvent
_finding = _suppression._finding
_is_comment_or_blank = _suppression._is_comment_or_blank
_is_ignore_file = _suppression._is_ignore_file
_is_relevant_gitignore_pattern = _suppression._is_relevant_gitignore_pattern
...
def git_suppression_findings(root: Path) -> list[Finding]:
    ...
    try:
        ...
        return [Finding("error", "git-suppression-scan-failed", root, str(exc), "tooling")]
    ...
    events_by_diff: dict[bool, list[_DiffEvent]] = {}
    accepted_pairs_by_diff: dict[bool, set[tuple[Path, Path]]] = {}
    provider_pairs_by_diff: dict[bool, set[tuple[Path, Path]]] = {}
    try:
        for cached in (False, True):
            events_by_diff[cached] = _git_diff_events(
                _run_git_diff(repository, cached=cached), repository
            )
            high_confidence = _git_rename_pairs(
                _run_git_rename_inventory(repository, cached=cached), repository
            )
            accepted = high_confidence | _candidate_rename_pairs(
            ...
            accepted_pairs_by_diff[cached] = {
                ...
                if _is_destructive_path(pair[0], repository)
                and _is_destructive_path(pair[1], repository)
            }
    except GitInventoryError as exc:
        return [Finding("error", "git-suppression-scan-failed", root, str(exc), "tooling")]
    try:
    ...
    for cached, events in events_by_diff.items():
        ...
        for event in events:
            ...
            if event.kind == "deleted":
                if (
                    _is_destructive_path(event.path, repository)
            ...
            if event.kind == "added":
                ...
                event.kind == "removed"
                and event.path not in rename_sources
                and _is_script_path(event.path)
                and _provider_invocation(event.text)
            ):
        ...
        for path, line_number, text in _provider_removals_for_rename(
    ...
    try:
        untracked_candidates = _untracked_candidates(repository, root)
    except GitInventoryError as exc:
        return [Finding("error", "git-suppression-scan-failed", root, str(exc), "tooling")]
