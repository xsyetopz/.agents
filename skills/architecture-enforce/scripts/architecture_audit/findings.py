...
def filename_findings(path: Path, root: Path) -> list[Finding]:
    ...
    if len(owner_words) >= 2 and owner_words[-1] in GENERIC_PROCEDURAL_SUFFIXES:
        findings.append(Finding("warning", "procedural-suffix", path, f"filename ends with generic procedural suffix '{owner_words[-1]}'; name the durable capability instead", "inventory"))
...
def _filesystem_child_directories(parent: Path) -> list[Path]:
    try:
        return [item for item in parent.iterdir() if item.is_dir() and item.name not in DEFAULT_IGNORED_DIRS]
    except OSError:
        return []
...
def directory_findings(
    ...
    *,
    ...
    for parent, children in sorted(by_parent.items(), key=lambda pair: str(pair[0])):
        ...
        phase_files = [child for child in children if semantic_words(child) in {(phase,) for phase in PROCEDURAL_PHASES}]
        if len(phase_files) >= len(PROCEDURAL_PHASES):
            for child in sorted(phase_files, key=str):
                phase = semantic_words(child)[0]
                findings.append(Finding("warning", "procedural-suffix", child, f"single-token procedural phase '{phase}' is not a durable capability boundary", "inventory"))
        ...
        if len(children) >= MICROFILE_MIN_SIBLINGS:
            try:
                all_micro = all(count_lines(child) <= MICROFILE_MAX_LINES for child in children)
            except OSError:
                all_micro = False
            if all_micro:
                findings.append(Finding("warning", "microfile-fragmentation", parent, f"flat directory contains {len(children)} microfiles; consolidate by durable capability", "inventory"))
        if parent != root and len(children) == 1:
            child_dirs = (
                [item for item in visible_child_dirs.get(parent, set()) if item.name not in DEFAULT_IGNORED_DIRS]
                if inventory is not None
    ...
    for grandparent, child_parents in sorted(by_grandparent.items(), key=lambda pair: str(pair[0])):
        ...
        if len(micro_dirs) >= MICROFILE_MIN_SIBLINGS:
            findings.append(Finding("warning", "microfile-fragmentation", grandparent, f"{len(micro_dirs)} sibling directories each contain one microfile; consolidate by durable capability", "inventory"))
        phase_dirs = [
        ...
        if len(phase_dirs) >= len(PROCEDURAL_PHASES):
            findings.append(Finding("warning", "procedural-directory", grandparent, f"phase directories {', '.join(sorted(item.name for item in phase_dirs))} split one microfile each; consolidate by durable capability", "inventory"))
            findings.append(Finding("warning", "microfile-fragmentation", grandparent, f"{len(phase_dirs)} procedural phase directories each contain one microfile; consolidate by durable capability", "inventory"))
    return findings
...
def package_manager_findings(
    ...
    def findings_for_inventory(visible: Sequence[Path]) -> list[Finding]:
        findings: list[Finding] = []
        package_dirs = sorted({path.parent for path in visible if path.name == "package.json"}, key=str)
        for base in package_dirs:
            names = {path.name for path in visible if path.parent == base}
            present = {manager for manager, locks in JS_LOCKFILES.items() if names.intersection(locks)}
            if len(present) > 1:
    ...
    findings: list[Finding] = []
    ignored_dirs = DEFAULT_IGNORED_DIRS | (OUTPUT_DIRECTORIES if repository is not None else set())
    for current, dirs, files in os.walk(root):
        base = Path(current)
        if repository is not None and base != root and (base / ".git").exists():
            dirs[:] = []
            continue
        dirs[:] = [name for name in dirs if name not in ignored_dirs and not (base / name / ".git").exists()]
