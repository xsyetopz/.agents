...
class AuditCliTests(AuditFixture, unittest.TestCase):
    ...
    def test_ignored_untracked_source_is_excluded_but_tracked_source_remains(self) -> None:
        with tempfile.TemporaryDirectory() as case_dir:
            case_root = Path(case_dir)
            ...
            paths = {
                ...
                for path in audit_architecture.iter_audited_files(case_root)
            }
            ...
            self.assertFalse(any(item["path"] == "src/ignored.ts" for item in payload["findings"]))
    ...
    def test_tracked_source_inside_ignored_directory_remains_auditable(self) -> None:
        with tempfile.TemporaryDirectory() as case_dir:
            ...
            run("config", "user.email", "audit@example.invalid")
            run("config", "user.name", "Architecture Audit")
            (case_root / ".gitignore").write_text("node_modules/**\n", encoding="utf-8")
    ...
    def test_ignored_package_manifests_are_not_scanned_for_lock_conflicts(self) -> None:
        with tempfile.TemporaryDirectory() as case_dir:
            ...
            run("config", "user.email", "audit@example.invalid")
            run("config", "user.name", "Architecture Audit")
            (case_root / ".gitignore").write_text(".architecture-enforcement.json\nignored/**\n", encoding="utf-8")
            run("add", ".gitignore")
            run("commit", "-qm", "baseline")
            ...
            self.assertNotIn("conflicting-lockfiles", {item["code"] for item in payload["findings"]})
    ...
    def test_tracked_package_lockfiles_remain_visible_to_package_checks(self) -> None:
        with tempfile.TemporaryDirectory() as case_dir:
            ...
            result = self.run_cli("--format", "json", root=case_root)
            self.assertEqual(result.returncode, 1)
            self.assertIn("conflicting-lockfiles", {item["code"] for item in self.json_output(result)["findings"]})
    ...
    def test_git_inventory_failure_does_not_fall_back_to_ignored_files(self) -> None:
        with tempfile.TemporaryDirectory() as case_dir:
            ...
            run("config", "user.email", "audit@example.invalid")
            run("config", "user.name", "Architecture Audit")
            (case_root / ".gitignore").write_text("ignored/**\n", encoding="utf-8")
            run("add", ".gitignore")
            run("commit", "-qm", "baseline")
            ignored = case_root / "ignored/source.ts"
            ignored.parent.mkdir()
            ignored.write_text("export const ignored = 1;\n", encoding="utf-8")
            real_run = subprocess.run

            def fail_discovery(command: list[str], *args: object, **kwargs: object) -> subprocess.CompletedProcess[str]:
                if command[:4] == ["git", "-C", str(case_root), "rev-parse"]:
                    raise subprocess.TimeoutExpired(command, 10)
                return real_run(command, *args, **kwargs)

            with patch("architecture_audit.discovery.subprocess.run", side_effect=fail_discovery), self.assertRaises(GitInventoryError):
                list(audit_architecture.iter_audited_files(case_root))

    def test_inventory_failure_does_not_read_ignored_policy_config(self) -> None:
        with tempfile.TemporaryDirectory() as case_dir:
            case_root = Path(case_dir)
            run = lambda *args: subprocess.run(["git", "-C", str(case_root), *args], check=True, capture_output=True, text=True)
            run("init", "-q")
            ...
            run("add", ".gitignore")
            run("commit", "-qm", "baseline")
            (case_root / ".architecture-enforcement.json").write_text("{\n", encoding="utf-8")
            with patch("architecture_audit.audit._git_inventory", side_effect=GitInventoryError("test failure")):
                report = audit_architecture.audit_report(case_root)
            codes = {item.code for item in report.findings}
            self.assertIn("git-inventory-failed", codes)
            self.assertNotIn("invalid-exception-file", codes)
    ...
    def test_tracked_deep_directories_are_used_for_topology_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as case_dir:
            ...
            run("commit", "-qm", "baseline")
            report = audit_architecture.audit_report(case_root)
            self.assertFalse(any(item.code == "single-file-directory" and item.path == case_root / "src/Open" for item in report.findings))
    ...
    def test_ignored_untracked_output_phases_are_not_audited(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as case_dir:
            ...
            self.assertFalse({f"dist/{phase}.ts" for phase in ("Open", "Reduce", "Commit")} & paths)
            result = self.run_cli("--format", "json", root=case_root)
            self.assertEqual(result.returncode, 0)
            payload = self.json_output(result)
            self.assertFalse({f"dist/{phase}.ts" for phase in ("Open", "Reduce", "Commit")} & set(payload["audited_paths"]))
            codes = {item["code"] for item in payload["findings"]}
