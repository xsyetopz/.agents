...
It supplements `languages.md`; it does not replace compiler or loader
requirements.
...
prove them. Use a canonical generated header/companion or an exact
`artifact_exemptions` record with provenance. Exemptions do not make equivalent
authored names acceptable and do not count excluded paths as full-repository
proof.
...
| 2009 | **Go** | Toolchain requirements plus Go package practice | `_test.go` is required for `go test` discovery. Preserve build-constraint OS/architecture suffixes and `doc.go`; use short lowercase/underscore stems without type-per-file ceremony. Follow the official module-layout guidance in `languages.md`. |
...
```json
{
  "naming_exceptions": [
    {
      "path": "src/protocol/required-public-name.ts",
      "rule": "semantic-token-limit",
      "reason": "published package export retained until the next major release",
      "owner": "protocol maintainers",
      "control": "package-export compatibility test",
      "review": "remove in v3 migration"
    }
  ],
  "artifact_exemptions": [
    {
      ...
      "reason": "generator output from the published protocol schema",
      "owner": "protocol maintainers",
      "control": "regeneration diff test",
      "review": "review on generator version upgrade"
    }
  ]
}
```
...
makes two colonies in the same current directory independently addressable.
Placeholder owners, `none` controls, `never` reviews, and similarly empty
rationales are invalid.

Artifact classes are `generated`, `vendor`, `schema-derived`, `migration`,
`snapshot`, `fixture`, and `framework`. An exact directory path may classify
its audited descendants. Overlapping artifact paths, missing targets, globs,
and artifact records with no audited files are errors.

## 8. Primary naming authorities
