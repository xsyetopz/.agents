...
from .tool_records import ToolResult
...
def _provider(tool: str):
    return next((item for item in PROVIDERS if item.provider_id == tool), None)


def _base_result(tool: str, process: ProcessResult, status: str, executable: str | None, version: str, argv: tuple[str, ...], diagnostics: tuple[str, ...] = (), payload: object | None = None) -> ToolResult:
    return ToolResult("package-graph", status, tool, executable, version, argv, process.exit_code, process.duration_ms, diagnostics=diagnostics, stdout_digest=process.stdout_digest, stderr_digest=process.stderr_digest, payload=payload)
...
def _cargo_graph(stdout: str) -> dict[str, object]:
    ...
    if not isinstance(payload, dict) or payload.get("version") != 1 or not isinstance(payload.get("packages"), list) or not isinstance(payload.get("workspace_members"), list):
...
def run_graph(root: Path, *, tool: str = "auto", timeout_s: float = 30) -> ToolResult:
    ...
    if process.status != "ok":
        ...
        return _base_result(tool, process, status, executable, version, argv, (process.message,))
    if process.exit_code != 0:
        return _base_result(tool, process, "tool-failed", executable, version, argv, (process.stderr.strip() or f"provider exited with status {process.exit_code}",))
    try:
        payload = _cargo_graph(process.stdout) if tool == "cargo-metadata" else _go_graph(process.stdout)
    except (json.JSONDecodeError, ValueError, TypeError) as error:
        return _base_result(tool, process, "invalid-output", executable, version, argv, (str(error),))
    return _base_result(tool, process, "passed", executable, version, argv, payload=payload)
