...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = run_ast_grep(Path(args.root), rule_id=args.rule_id, language=args.language, pattern=args.pattern, severity=args.severity, message=args.message, paths=tuple(args.paths), timeout_s=args.timeout)
    payload = result.as_dict(Path(args.root))
    if args.format == "json":
        print(json.dumps(payload, indent=2, sort_keys=True))
    else:
