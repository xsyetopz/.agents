...
def render_text(root: Path, files: Sequence[Path], findings: Sequence[Finding], excludes: Sequence[str] = ()) -> None:
...
def render_json(root: Path, report: AuditReport, excludes: Sequence[str] = ()) -> None:
    ...
    payload = {"root": str(root), "source_files": sum(path.suffix.lower() in SOURCE_EXTENSIONS for path in report.files), "audited_files": len(report.files), "excludes": list(excludes), "analyzers": [asdict(item) for item in report.analyzers], "findings": [{**asdict(item), "path": shown_path(item.path)} for item in report.findings]}
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.output_format == "json":
        render_json(root, report, args.exclude)
    else:
        render_text(root, report.files, report.findings, args.exclude)
