...
The scanner is deliberately limited to deterministic structural signals:
authored line-size bands, naming heuristics, generic/flat buckets, artifact
classification, JavaScript lockfile conflicts, and machine-readable exception
quality. It does not infer semantic ownership, dependency direction, quality
attributes, API compatibility, or runtime correctness. Use `--fail-on warning`
for a policy gate only when the repository has reviewed its thresholds; use
`--fail-on never` for an advisory inventory. Excluded and exempt paths remain
visible and cannot establish full-repository acceptance.
...
file and artifact discovery, `findings.py` for structural rules, `exceptions.py`
for machine-readable exceptions, `audit.py` for orchestration, and `cli.py` for
...
Run `python3 scripts/architecture_tools.py capabilities --root <repo>` before
using a configured provider. The architecture audit exposes each configured
provider status in its text/JSON report. A required provider that is absent,
