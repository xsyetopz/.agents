...
| 1986 | **Erlang** | Use OTP applications and supervision trees as operational boundaries. Modules own behavior, processes own state, and behaviours define contracts. | Keep `src/`, `include/`, `priv/`, and `test/` within each application. Colocate EUnit only for unit-local behavior and use Common Test for system interactions. | A flat module namespace across unrelated OTP apps, process dictionaries as shared state, cross-app calls to private modules, or supervisors containing business logic. |
...
| 2001 | **D** | Use Dub packages/configurations and D modules as boundaries; match module declarations to source paths and use `package.d` only as an intentional facade. | Follow `source/`, `tests/`, or repository-native `unittest` blocks; keep substantial integration suites outside production modules. | One-type-per-file ceremony, an automatic re-exporting `package.d`, or C-style global prefixes where module qualification provides ownership. |
...
| 2010 | **Rust** | Crates are build/reuse boundaries; modules and visibility enforce internal ownership. Keep `lib.rs`/`main.rs` as composition surfaces and use `pub(crate)` before widening API. | Use `foo.rs` plus `foo/` submodules or established `foo/mod.rs`; keep unit tests source-adjacent and public-boundary integration tests under `tests/`. | Crate-per-file fragmentation, indiscriminate `pub use`, giant prelude modules, circular feature flags, or splitting solely to evade borrow/visibility design. |
...
| 2008 | **Nim** | Use Nim modules, packages, and build targets as visibility and dependency boundaries; isolate C/Objective-C interop and generated bindings at adapters. | Follow the detected Nimble/project layout and module import mapping; keep tests in dedicated suites or source-adjacent `unittest` blocks according to the runner. | Global compile-time state used as architecture, broad `include` chains that bypass imports, speculative OO layers, or generated C output treated as authored policy. |
...
| 2016 | **Zig** | Use build modules/artifacts and explicit imports as boundaries. Keep `root.zig`/`main.zig` as composition surfaces and capability submodules explicit. | Use inline `test` blocks for local behavior and owned test roots/artifacts for integration; isolate generated bindings. | Generic `util.zig`, public declarations by accident, one build step owning unrelated products, or C ABI details spread through domain modules. |
