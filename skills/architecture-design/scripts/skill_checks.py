...
PLACEHOLDER_RE = re.compile(
    r"(?:<[^>]+>|\b(?:TBD|TODO|FIXME|PLACEHOLDER)\b)", re.IGNORECASE
)
ID_PATTERNS = {
...
def evaluate(text: str, mode: str) -> tuple[list[str], list[str]]:
    ...
    candidate_headings = re.findall(
        r"^#{2,6}\s+Candidate\b", text, re.IGNORECASE | re.MULTILINE
    )
    baseline_present = bool(re.search(r"do[- ]less|baseline", text, re.IGNORECASE))
    ...
    if qa_rows < min_qa:
        errors.append(
            f"{mode} requires at least {min_qa} quality-attribute scenario references; found {qa_rows}."
        )
    ...
    if mode in {"R2", "R3", "R4"} and "```mermaid" not in text:
        warnings.append(
            "No Mermaid diagram found; verify that static and dynamic views are still unambiguous."
        )

    consequence_terms = ["negative", "disadvantage", "liabilit", "accepted cost"]
    if mode in {"R1", "R2", "R3", "R4"} and not any(
        term in text.lower() for term in consequence_terms
    ):
    ...
    if (
        re.search(r"\b(?:Manager|Service|Handler|Engine|Utils)\b", text)
        and "purpose" not in text.lower()
    ):
        warnings.append(
            "Generic component nouns appear without an obvious purpose/contract section."
        )
...
def _parser() -> argparse.ArgumentParser:
    ...
    report = commands.add_parser(
        "report",
        aliases=["architecture-report"],
        help="validate an architecture report",
    )
