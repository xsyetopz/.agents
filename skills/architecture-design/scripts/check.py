...
def check_required_files(contract: dict[str, object], errors: list[str]) -> None:
    ...
    if not isinstance(required, list) or required != REQUIRED_FILES:
...
def check_yaml(errors: list[str], skill_name: str) -> None:
    ...
    for line in lines:
        match = re.match(
            r"^\s*(display_name|short_description|default_prompt):\s*(.*?)\s*$", line
        )
        if match:
...
def check_evals(
    contract: dict[str, object], errors: list[str], skill_name: str
) -> None:
    ...
    if not isinstance(static, list) or not static:
        ...
        package_checks = [
            item
            for item in static
            if isinstance(item, dict) and item.get("id") == "package-contract"
        ]
        ...
        for item in static:
            if (
                ...
                or item.get("command") != "python3 scripts/check.py"
                or item.get("expect_exit") != 0
            ):
                errors.append(
                    "every static check must run python3 scripts/check.py with expect_exit 0"
                )
    ...
    for index, case in enumerate(cases):
        ...
        if set(case) != {"id", "prompt", "expected_outcome"}:
            errors.append(
                f"codex_cases[{index}] must contain only id, prompt, expected_outcome"
            )
    ...
    if (
        "positive" not in lower
        or not ("near-miss" in lower or "near_miss" in lower)
        or not any(word in lower for word in ("safety", "failure", "boundary"))
    ):
        errors.append(
            "codex_cases must include positive, near-miss, and safety/failure/boundary IDs"
        )
    ...
    if not isinstance(refs, list) or not refs:
        ...
        for raw in refs:
            ...
            if relative is None:
                continue
            if (
                not str(raw).startswith("references/")
...
def check_skill(errors: list[str], skill_name: str) -> str:
    ...
    if headings != HEADINGS:
        errors.append(
            "SKILL.md H2 headings must match the shared ordered structure: "
            + ", ".join(HEADINGS)
        )
...
def check_package_manifest(errors: list[str]) -> None:
    """Verify the package inventory and digests without external dependencies."""
    manifest = ROOT / "PACKAGE-MANIFEST.sha256"
    if not manifest.is_file():
        errors.append("missing required file: PACKAGE-MANIFEST.sha256")
        return
    ...
    try:
        ...
        errors.append(f"PACKAGE-MANIFEST.sha256 cannot be read: {exc}")
        return
    ...
    for line_no, line in enumerate(lines, 1):
        ...
        if match is None:
            errors.append(f"PACKAGE-MANIFEST.sha256 malformed line {line_no}")
            continue
        digest, raw_path = match.groups()
        if not raw_path.startswith("./"):
            errors.append(
                f"PACKAGE-MANIFEST.sha256 path must be relative on line {line_no}"
            )
            continue
        relative = safe_path(raw_path[2:], "PACKAGE-MANIFEST.sha256 entry", errors)
        if relative is None:
            continue
        normalized = relative.as_posix()
        if normalized == "PACKAGE-MANIFEST.sha256":
            errors.append("PACKAGE-MANIFEST.sha256 must not hash itself")
            continue
        if normalized in listed:
            errors.append(f"PACKAGE-MANIFEST.sha256 duplicate entry: {raw_path}")
            continue
    ...
    if set(listed) != set(expected):
        ...
        if missing:
            errors.append(
                "PACKAGE-MANIFEST.sha256 missing files: " + ", ".join(missing)
            )
        if extra:
            errors.append(
                "PACKAGE-MANIFEST.sha256 lists unexpected files: " + ", ".join(extra)
            )
    for relative, path in expected.items():
        actual = hashlib.sha256(path.read_bytes()).hexdigest()
        if listed.get(relative) != actual:
            errors.append(f"PACKAGE-MANIFEST.sha256 digest mismatch: ./{relative}")
