...
the `architecture-enforce` capability preflight and full audit before editing
and again after editing:
...
The audit uses full filesystem scope, fixed thresholds, and failure on every
warning or error. Record commands, scope, provider status, findings, and exit
codes. A failed or blocked preflight/audit stops the design until its owning
issue is resolved.
