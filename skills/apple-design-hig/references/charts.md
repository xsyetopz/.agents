...
*Image description: A stylized representation of a bar chart. The image is tinted red to subtly reflect the red in the original six-color Apple logo.*
...
*Image description: A bar chart with callouts that identify chart components, such as axes, grid lines, marks, ticks, axis value labels, and the overall plot area.*
...
*Image description: A bar chart that depicts the number of steps for each day in a month.*
...
*Image description: A line chart that depicts the performance of a stock over a five-year period.*
...
*Image description: A point-mark chart that depicts per-day readings of heartbeats per minute over a 5 1/2-month period.*
...
*Image description: An illustration of Battery Settings, which uses a chart to depict battery charge over time, where the charge can vary within a fixed range from 0% to 100%.*
...
*Image description: An illustration of the Steps chart in the Health app, which shows the average number of steps per day for a single week. Weekly range*

*Image description: An illustration of the Steps chart in the Health app, which shows the average number of steps per day in a one-month period. Monthly range*
...
*Image description: An illustration of the rain forecast for the next hour in the Weather app, which uses succinct, plain language to describe the expected precipitation.*
...
*Image description: An illustration of a blood pressure chart in the Health app that uses a red circle to represent systolic values and a black or white diamond to represent diastolic values.*
...
*Image description: An illustration of iPhone Storage Settings, which uses a single bar mark containing several segments of different colors to show the relative space taken by items such as music, apps, and photos. The bar includes a narrow strip of empty space between each pair of segments.*
...
*Image description: An illustration of a chart in Maps that shows the range of elevations over the total distance of the trip. A VoiceOver focus indicator is visible on top of the chart, containing approximately one-fifth of the total distance and elevation. For the focused section of this cycling elevation chart, VoiceOver provides information about that portion of the route, including distance and elevation changes.*
...
- [accessibilityRespondsToUserInteraction(_:)](https://developer.apple.com/documentation/SwiftUI/View/accessibilityRespondsToUserInteraction(_:))
