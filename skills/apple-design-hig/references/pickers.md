...
*Image description: A stylized representation of a selected item in a scrollable list. The image is tinted red to subtly reflect the red in the original six-color Apple logo.*
...
*Image description: An illustration of a compact date picker, with a single inline row showing the currently selected date. The picker opens as a popover extending down from the row, and includes a full calendar month for choosing the date. In a compact layout, a picker opens as a popover over your content.*

*Image description: An illustration of an inline date picker, titled 'Date'. A toggle at the top is switched on, and a calendar month for choosing the date appears below the title and toggle. In an inline layout, a picker opens inline with your content.*

*Image description: An illustration of an inline time picker, titled 'Time'. The currently selected time appears in the title row, and three vertical wheels appear below the title row for choosing the hour, minute, and AM or PM value. Another example of an inline picker uses wheels to choose values for date and time.*
...
*Image description: An illustration representing a screen containing a picker view on Apple Watch, showing three items in a list. The center item is highlighted.*

*Image description: An illustration representing a screen containing a date picker on Apple Watch, with the day highlighted.*

*Image description: An illustration representing a screen containing a time picker on Apple Watch, with the minutes highlighted.*
...
*Image description: An illustration representing a screen that contains a picker button on Apple Watch. The button’s text denotes that the second item is selected.*

*Image description: An illustration representing a screen showing a list of items on Apple Watch. The second item in the list is selected.*
