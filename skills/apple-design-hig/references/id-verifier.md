...
*Image description: A sketch of progressively larger curved lines emerging from the bottom corner of an ID card, suggesting ID Verifier. The image is overlaid with rectangular and circular grid lines and is tinted blue to subtly reflect the blue in the original six-color Apple logo.*
...
| --- | --- |
| *Image description: An illustration of a Verify Age button.* | An app that checks whether people are old enough to attend an event or access a venue, like a concert hall. |
| *Image description: An illustration of a Verify Identity button.* | An app that verifies whether specific identity information matches expected values, such as name and birth date when picking up a rental car. |
...
- [ageAtLeast(_:)](https://developer.apple.com/documentation/ProximityReader/MobileDriversLicenseDataRequest/Element/ageAtLeast(_:))
