...
*Image description: A sketch of a keyboard, suggesting keyboard input. The image is overlaid with rectangular and circular grid lines and is tinted purple to subtly reflect the purple in the original six-color Apple logo.*
...
| --- | --- | --- |
| Command | *Image description: Outline of a stylized clover shape.* | Prefer the Command key as the main modifier key in a custom keyboard shortcut. |
| Shift | *Image description: Outline of an upward-pointing arrow.* | Prefer the Shift key as a secondary modifier that complements a related shortcut. |
...
*Video description: A recording that shows two hands typing on a physical keyboard while the person runs an app in visionOS. A virtual window is visible above the physical keyboard, and displays the entered text and suggestions.*
