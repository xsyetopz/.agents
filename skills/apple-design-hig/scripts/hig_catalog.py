...
def fetch_json(base_url: str, slug: str, timeout: float) -> dict:
    ...
    if not isinstance(payload, dict):
        raise RuntimeError(f"Apple HIG returned a non-object JSON payload: {url}")
    return payload
...
def hig_identifier(value: object) -> bool:
    return isinstance(value, str) and (
        value == ROOT_IDENTIFIER
        or value.startswith(HIG_PREFIX)
        or value.startswith(HIG_PREFIX_LOWER)
    )
...
def topic_records(payload: dict) -> dict[str, dict[str, str]]:
    ...
    for node in walk(payload):
        ...
        if not hig_identifier(identifier):
            continue
