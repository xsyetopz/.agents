...
LEGACY_MARKER = "scaffold-agent-governance:"
LEGACY_TOKENS = (
    "HUMAN-AUTHORED",
    "AI-ASSISTED",
...
def _inline_body_matches(start: str, body: str) -> bool:
    lines = body.strip("\r\n").splitlines()
    if start == "<!-- scaffold-agent-governance:start -->":
        return lines == list(ROOT_BLOCK_LINES)
    if start == "<!-- scaffold-agent-governance:pr:start -->":
        return lines == list(PR_BLOCK_LINES)
    if start == "<!-- scaffold-agent-governance:readme:start -->":
        return lines == list(README_BLOCK_LINES)
    if start == "<!-- scaffold-agent-governance:translation:start -->":
        return _translation_block_matches(lines)
    if start == "# scaffold-agent-governance:codeowners:start":
...
def recognized_legacy(path: Path, root: Path) -> bool:
    ...
    html_marker = "<!-- scaffold-agent-governance:file -->" in lines
    yaml_marker = "# scaffold-agent-governance:file" in lines
    ...
    if relative.startswith("docs/ai-contribution-policy.") and relative.endswith(".md"):
        return (
            "<!-- scaffold-agent-governance:translation:start -->" in lines
            and "<!-- scaffold-agent-governance:translation:end -->" in lines
            and any(
