...
STATIC_KEYS = {"id", "command", "expect_exit"}
...
def check_skill(errors: list[str], contract: dict[str, object]) -> None:
    ...
    if set(frontmatter) != {"name", "description"}:
        add(errors, "SKILL.md: frontmatter must contain only name and description")
    ...
    section_matches = list(re.finditer(r"(?m)^##[ \t]+(.+?)[ \t]*$", body))
    ...
    for index, match in enumerate(section_matches):
        ...
        section_text[match.group(1).strip()] = body[match.end() : end].strip()
    for heading in EXPECTED_HEADINGS:
        if not section_text.get(heading):
            add(errors, f"SKILL.md: section {heading!r} must contain content")
    resources = section_text.get("Resources", "")
    first_resource = next(
        (line.strip() for line in resources.splitlines() if line.strip()), ""
    )
    if "(references/index.md)" not in first_resource:
        add(errors, "SKILL.md: Resources must start with references/index.md")
    verify = section_text.get("Verify", "")
    if "python3 scripts/check.py" not in verify:
        add(errors, "SKILL.md: Verify must name python3 scripts/check.py")
    if not re.search(r"\b(?:UNVERIFIED|unavailable)\b", verify, re.IGNORECASE):
        add(errors, "SKILL.md: Verify must classify unavailable evidence")
...
def check_contract(errors: list[str]) -> dict[str, object]:
    ...
    if set(payload) != CONTRACT_KEYS:
        add(errors, "assets/contract.json: keys must match the common contract")
    if payload.get("schema_version") != 1 or isinstance(
        payload.get("schema_version"), bool
    ):
        add(errors, "assets/contract.json: schema_version must be integer 1")
    ...
    if isinstance(references, list) and len(set(map(str, references))) != len(
        references
    ):
        add(errors, "assets/contract.json: reference_paths must be unique")
...
def check_evals(errors: list[str], contract: dict[str, object]) -> None:
    ...
    if set(payload) != EVAL_KEYS:
        add(errors, "evals/evals.json: keys must match the evaluation contract")
    if payload.get("schema_version") != 1 or isinstance(
        payload.get("schema_version"), bool
    ):
        add(errors, "evals/evals.json: schema_version must be integer 1")
    if payload.get("skill_name") != ROOT.name:
    ...
    elif any(not isinstance(item, dict) or set(item) != STATIC_KEYS for item in static):
        add(
            errors,
            "evals/evals.json: static records must contain only id, command, expect_exit",
        )
    elif not any(
    ...
    for index, case in enumerate(cases):
        ...
        if set(case) != CASE_KEYS:
            add(errors, f"{label}: keys must contain only id, prompt, expected_outcome")
        case_id = case.get("id")
...
def check_biome_template(errors: list[str]) -> None:
    payload = load_json(ROOT / "assets" / "biome.json", errors)
    if not isinstance(payload, dict):
        add(errors, "assets/biome.json: expected an object")
        return
    expected_schema = "https://biomejs.dev/schemas/2.5.9/schema.json"
