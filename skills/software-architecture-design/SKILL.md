...
description: Design, audit, enforce, or migrate software architecture when work changes module ownership, dependency direction, public boundaries, package topology, generated-code ownership, or test placement. Use before structural edits that cross components; skip only for bounded changes that preserve existing boundaries.
---
...
1. Map current topology, a do-less baseline, and materially different candidates; compare migration cost, reversibility, operational risk, and measurable quality attributes.
2. Select the smallest cohesive structure and assign each changed path one durable owner, responsibility, visibility, lifecycle, dependency direction, and reason it remains separate.
3. Implement the authorized slice through canonical source and repository-native configuration, changing generator inputs rather than generated output and preserving public entrypoints unless migration is explicit.

## Validation

1. Run `python3 scripts/providers.py capabilities --root <repo> --format json`, `python3 scripts/audit_architecture.py <repo> --format json`, focused tests, production or integration entrypoints, architecture checks, and final diff inspection; return unresolved external or runtime evidence as `UNVERIFIED`.
...
- Repository governance, pipeline implementation, and prompt/tool routing are separate concerns. Handle them directly when they are part of the authorized request, but do not stop to locate or install a companion skill.
