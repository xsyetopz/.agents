...
Use only sections justified by the application. For a named model, read the
matching official snapshot from `official-sources.md` first, then remove
repeated instructions, irrelevant tools, and examples without a product
requirement or measured gap. GPT-5.6 is one optional family route, not a
