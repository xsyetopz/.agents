...
AUDIT_MODEL = os.environ.get("PROMPT_ENGINEERING_MODEL", "gpt-5.6-sol")
...
SCENARIOS = (
    Scenario(
        ...
        lambda text: all(
            (
                re.search(r"official OpenAI|OpenAI.*official", text, re.IGNORECASE),
                re.search(
                    r"state each instruction once|one place|duplicat",
                    text,
                    re.IGNORECASE,
                ),
                re.search(
                    r"same (?:representative )?(?:eval|case|task)|baseline.*candidate|candidate.*baseline",
                    text,
                    re.IGNORECASE | re.DOTALL,
                ),
                re.search(
                    r"example.*measured.*(?:gap|failure)|measured.*(?:gap|failure).*example|product requirement",
                    text,
                    re.IGNORECASE | re.DOTALL,
                ),
                re.search(
                    r"local changes|safe local|without asking", text, re.IGNORECASE
                ),
            )
        ),
    ),
    Scenario(
        ...
        lambda text: all(
            (
                re.search(
                    r"investigat|report|read[- ]only|diagnos", text, re.IGNORECASE
                ),
                re.search(r"edit|mutat|change", text, re.IGNORECASE),
                re.search(
                    r"unauthor|unapproved|without|not author", text, re.IGNORECASE
                ),
            )
        ),
    ),
    Scenario(
        ...
        lambda text: (
            bool(
                re.search(
                    r"over[- ]delegat|unnecessary (?:worker|delegat)|coordination (?:cost|overhead|benefit)|not justified|without (?:concrete )?independent",
                    text,
                    re.IGNORECASE,
                )
            )
    ...
    Scenario(
        ...
        ("prompt.md",),
        lambda text: bool(
            re.search(r"prompt\.md|changed|updated|revised", text, re.IGNORECASE)
        ),
    ),
)
...
def baseline_skill() -> str:
    """Load the package-local baseline; never depend on a checkout outside it."""
    baseline = SKILL_ROOT / "references" / "audit-baseline.txt"
    if baseline.is_file():
        return baseline.read_text(encoding="utf-8")
    # A copied package may intentionally omit optional audit fixtures. Keep the
    # command usable while making the absence visible in the report through an
    # identical baseline/candidate comparison.
    return (SKILL_ROOT / "SKILL.md").read_text(encoding="utf-8")


def run(condition: str, instructions: str, scenario: Scenario) -> tuple[bool, str]:
    with tempfile.TemporaryDirectory(
        prefix=f"prompt-engineering-{condition}-{scenario.name}-"
    ) as raw:
        root = Path(raw)
        ...
        subprocess.run(
            ["git", "config", "user.name", "Prompt Audit"], cwd=fixture, check=True
        )
        ...
        command = [
            ...
            "-c",
            f"model_instructions_file={json.dumps(str(instruction_file))}",
            "-c",
            ...
            "-c",
            f"shell_environment_policy.set={{HOME={json.dumps(str(fixture))}}}",
            "-c",
        ...
        env = {
            ...
            if key in {"PATH", "HOME", "CODEX_HOME", "TMPDIR"}
        }
        try:
        ...
        if scenario.name == "direct-edit":
            ...
            content_ok = (
                ...
                and bool(
                    re.search(
                        r"installed.{0,40}codex", combined, re.IGNORECASE | re.DOTALL
                    )
                )
                and "baseline" in combined.lower()
                and "candidate" in combined.lower()
            )
        passed = effects_ok and content_ok
        detail = f"paths={changed}, answer_bytes={len(answer.encode())}"
        if not passed:
            detail += f", answer={answer[:1800]!r}"
...
def main() -> None:
    conditions = {
        ...
        "candidate": (SKILL_ROOT / "SKILL.md").read_text(encoding="utf-8"),
    }
    ...
    for scenario in SCENARIOS:
        for condition in ("baseline", "candidate"):
            ...
            details.append(
                f"{condition}/{scenario.name}: {'PASS' if passed else 'FAIL'} ({detail})"
            )
    failures = []
    for scenario in SCENARIOS:
