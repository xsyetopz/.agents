    ...
    for source, text in sources:
        for match in LINK.finditer(text):
            ...
            try:
                ...
                fail(
                    errors, f"{source.relative_to(ROOT)} link leaves package: {target}"
                )
                continue
...
def check_skill(text: str, contract: dict, refs: list[str], errors: list[str]) -> None:
    ...
    if headings != expected:
        fail(
            errors,
            f"SKILL.md: level-two headings must be {expected!r}; got {headings!r}",
        )
    ...
    for relative in refs:
        if relative not in mapped:
            fail(
                errors,
                "SKILL.md or references/index.md does not route reference: " + relative,
            )
