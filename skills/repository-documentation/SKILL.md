...
description: Use this skill when creating or auditing README, CHANGELOG, release notes, CONTRIBUTING, AGENTS, CODEOWNERS, templates, or repository governance; use runtime engineering, Git, CI/CD, or a dedicated skill-authoring workflow for adjacent work.
---
...
3. Load only the matching documentation or governance reference from the direct routes below.
   - [CHANGELOG.md](references/docs-changelog.md) · [README.md](references/docs-readme.md) · [Repo Docs sources and provenance](references/docs-sources.md) · [Agent governance](references/governance-agent-governance.md)
...
- Prose does not enforce what tooling never reads, and unavailable settings, owners, contacts, benchmarks, or bypass actors remain `UNVERIFIED`.
- Publication and hosted mutation require exact authorization after a preview.
- Route local Git and team policy to `$git-workflows`, pipelines to `$git-ci-cd`, hosted APIs to `$git-actions`, and prose-style or reusable-skill work to the matching dedicated workflow.
- Edit established human-readable formats directly rather than generating custom schemas, manifests, registries, or governance trees.
